...
Multi-crate Cargo workspace that clears 'WDA_EXCLUDEFROMCAPTURE' from Windows display-affinity flags via classic LoadLibrary DLL injection — 'OpenProcess' → 'VirtualAllocEx' → 'WriteProcessMemory' → 'CreateRemoteThread(LoadLibraryA)'. Six crates: shared injection lib, CLI, egui GUI, one-shot payload DLL, persistent payload DLL, and stress tester. GUI features live process list, auto-inject + tray mode, persistent re-strip (fights back on a timer), watch-list for auto-injecting on process start, global hotkey, toast notifications, injection log, update checker, and full browser multi-process enumeration. 32-bit fallback path for injecting into x86 targets. Ships with an Inno Setup installer.
